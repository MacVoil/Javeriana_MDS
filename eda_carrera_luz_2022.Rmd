...
**Ingeniería de características y limpieza** {data-navmenu="Menu"}
==================================
...
carrera_clean <- CarreraLuz22 %>% 
  ...
  mutate(timerum_minutes = timerun/60, 
         time_over_mean = timerum_minutes > median(timerum_minutes),
         rango_edad = cut_number(edad, 5),
         origen_refact = fct_lump_lowfreq(origen),
         nacionalidad_refact = fct_lump_lowfreq(nacionalidad))

ggplot(carrera_clean , aes( y = timerum_minutes)) +
  stat_halfeye(adjust = 0.5,
               justification = -0.2,
               .width = 0,
               point_colour = NA,
...
### **Tiempos por arriba (TRUE) y por bajo (FALSE) de la mediana**

```{r}
g <- ggplot(carrera_clean , 
            aes(x = time_over_mean, fill = time_over_mean)) +
  geom_bar() +
  theme_tq() +
  scale_fill_tq(theme = "light") +
  theme(legend.position = "none") +
...
**Embudo de Correlación** {data-navmenu="Menu"}
==================================
...
correlated_carrera <- bin_car %>%
  correlate(target = time_over_mean__0)


g <- correlated_carrera %>%
    filter(!(feature %in% c("time_over_mean"))) %>%
